import {calendarSignInRebate, isCalenderSignInRebateDone} from "@/apis";
import React, {useEffect, useState} from "react";

// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-expect-error
export function CalendarSignIn({handleRefresh}) {

    const [sign, setSign] = useState(false);

    const calendarSignInRebateHandle = async () => {
        if (sign){
            window.alert("今日已签到！")
            return;
        }
        const queryParams = new URLSearchParams(window.location.search);
        const result = await calendarSignInRebate(String(queryParams.get('userId')));
        const {code, info}: { code: string; info: string; } = await result.json();

        if (code != "0000" && code != "0003") {
            window.alert("日历签到返利接口，接口调用失败 code:" + code + " info:" + info)
            return;
        }

        setSign(true);

        // 设置一个3秒后执行的定时器
        const timer = setTimeout(() => {
            handleRefresh()
        }, 150);

        // 清除定时器，以防组件在执行前被卸载
        return () => clearTimeout(timer);
    }

    const isCalenderSignInRebateDoneHandle = async () => {

        const queryParams = new URLSearchParams(window.location.search);
        const result = await isCalenderSignInRebateDone(String(queryParams.get('userId')));
        const {code, info, data}: { code: string; info: string; data: boolean } = await result.json();

        if (code != "0000") {
            window.alert("判断是否签到接口，接口调用失败 code:" + code + " info:" + info)
            return;
        }

        setSign(data);
    }

    const currentDate = new Date();
    const year = currentDate.getFullYear();
    const month = (`0${currentDate.getMonth() + 1}`).slice(-2); // 月份是从0开始的
    const day = (`0${currentDate.getDate()}`).slice(-2);

    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const formattedDate = `${year}-${month}-${day}`;

    useEffect(() => {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        isCalenderSignInRebateDoneHandle().then(r => {
        });
    }, [])

    return (
        <>
            <div
                className="px-6 py-2 mb-8 text-white bg-green-600 rounded-full shadow-lg hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-blue-300"
                style={{cursor: "pointer"}}
                onClick={calendarSignInRebateHandle}
            >
                {sign ? "今日已签到" : "点击签到「获得抽奖次数」"}
            </div>
        </>
    )

}
